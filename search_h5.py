        ...
        if isinstance(item, h5py.Group):
            ...
            try:
                ...
                if hasattr(value, 'shape') and value.shape != ():  # not scalar
                    ...
                    flat_value = [float(x) if not (isinstance(x, float) and math.isnan(x)) else "nan" for x in flat_value]
...
def main():
    ...
    parser.add_argument("-v", "--values", action="store_true", help="Search in values instead of keys")
